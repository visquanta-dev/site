...
export default function BlogCard({ post }: { post: BlogPost }) {
    ...
    return (
        <Link href={`/blog/${post.slug}`} className="group relative block h-full w-full">
            <motion.div
                className="h-full w-full flex flex-col overflow-hidden rounded-2xl bg-[#111111] border border-white/[0.06] transition-all duration-400 ease-[cubic-bezier(0.4,0,0.2,1)] group-hover:border-[#ff7404]/25 group-hover:shadow-[0_20px_40px_rgba(0,0,0,0.3),_0_0_30px_rgba(255,116,4,0.08)] group-hover:-translate-y-1.5"
            >
                {/* Image Area */}
                ...
                <div className="flex flex-col flex-1 p-6">
                    ...
                    {/* Headline */}
                    <h3 className="text-[18px] font-semibold text-white leading-[1.4] mb-2 line-clamp-2 group-hover:text-[#ff7404] transition-colors">
                        {post.headline}
                    </h3>
                    ...
                    <div className="mt-auto pt-4 flex items-center text-[#ff7404] text-[14px] font-medium">
                        <span className="relative inline-block pb-0.5">
                            Read Article
                            <span className="absolute left-0 bottom-0 w-full h-[1px] bg-[#ff7404] origin-left scale-x-0 transition-transform duration-300 group-hover:scale-x-100"></span>
                        </span>
                    </div>
                </div>
            </motion.div>
